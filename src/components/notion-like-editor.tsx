...
const MENU_ITEMS = [
  { id: 'text', name: 'Text', icon: 'Aa', description: 'Just start writing with plain text.' },
  { id: 'page', name: 'Page', icon: '📄', description: 'Embed a sub-page inside this page.' },
  { id: 'todo', name: 'To-do list', icon: '☐', description: 'Track tasks with a to-do list.' },
  { id: 'heading1', name: 'Heading 1', icon: 'H1', description: 'Big section heading.' },
  { id: 'heading2', name: 'Heading 2', icon: 'H2', description: 'Medium section heading.' },
  { id: 'heading3', name: 'Heading 3', icon: 'H3', description: 'Small section heading.' },
  { id: 'table', name: 'Table', icon: '▦', description: 'Add simple tabular content to your page.' },
];
...
const NotionLikeEditor = () => {
  ...
  return (
    ...
    >
      {blocks.map((block, index) => (
        <div key={block.id} className="relative">
          <textarea
            ...
            value={block.content}
            onChange={(e: ChangeEvent<HTMLTextAreaElement>) => handleContentChange(block.id, e.target.value)}
            onKeyDown={(e: KeyboardEvent<HTMLTextAreaElement>) => handleKeyDown(e, block.id)}
            onCompositionStart={handleCompositionStart}
            onCompositionEnd={handleCompositionEnd}
            placeholder={index === 0 ? "Type '/' for commands" : "Continue typing..."}
            rows={1}
            style={{
              minHeight: '1.5rem',
              lineHeight: '1.5rem',
      ...
      {menuOpen && (
        <div
          className="absolute top-4 left-4 bg-gray-800 shadow-lg rounded-md border border-gray-700 p-2 z-10 w-64"
        >
          <div className="text-gray-400 text-sm font-semibold mb-2 px-2">Basic blocks</div>
          {MENU_ITEMS.map(item => (
            <div
              key={item.id}
            ...
            >
              <span className="mr-2 text-white bg-gray-600 rounded p-1 text-xs">{item.icon}</span>
              <div>
